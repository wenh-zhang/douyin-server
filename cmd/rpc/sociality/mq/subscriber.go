...
func (s *Subscriber) Subscribe() (chan *FollowWithAction, func(), error) {
	closeConnFunc := func() {
	...
	followChan := make(chan *FollowWithAction)
	go func() {
		for msg := range msgs {
			info := new(FollowWithAction)
	...
	return followChan, closeFunc, nil
}
